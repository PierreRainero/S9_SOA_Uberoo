...
curl -X POST -H "Content-Type:application/JSON; charset=UTF-8" -d "{\"route\":\"http://localhost:8080/restaurants/orders/\"}" "http://localhost:5001/subscribe" # Subscription of the restaurant service to the bus
curl -X POST -H "Content-Type:application/JSON; charset=UTF-8" -d "{\"route\":\"http://localhost:8080/deliveries\"}" "http://localhost:5001/subscribe" # Subscription of the coursier service to the bus
